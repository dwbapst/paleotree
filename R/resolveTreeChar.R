...
#' @rdname
#' @export
resolveTreeChar<-function(tree, trait, ordered=TRUE, stateBias="primitive", type="MPR", cost=NULL){
	#  ordered=TRUE; type="MPR"; cost=NULL; stateBias="primitive"
	#ordered=TRUE : put clades together relative to character being ordered, 0 is most primitive state
	...
	for(i in 1:length(polyParts)){	
		...
		#
		ancMat<-ancPropStateMat(trait, tree1, ordered=ordered, type=type, cost=cost)
		...
		if(ordered & (stateBias=="derived" | stateBias=="primitive")){
		...
		if((ordered & stateBias=="none") | (!ordered)){				
		...
		if(length(groupings)>1){
			...
			if(ordered){
				...
				for(j in 1:length(grouping)){
					...
					edgar<-rbind(edgar,c(bottom,newsie))	#bottom never changes
					#need to update bottom and newsie
		...
		if(Ntip(tree1)!=Ntip(tree3)){stop("Taxa loss/added somehow in fixing polytomies?")}
		tree1<-tree3
		}
	#randRes remaining unresolved areas: yes, no?
		#no, leave this alone, can be a second line of code, for god sakes
	treeFinal<-ladderize(treeFinal,right=FALSE)
	return(treeFinal)
	}
