...
testEdgeMat <- function(tree){
    ...
    if(any(tree$edge>expNodeNumber)){
        stop(
            paste0("Some elements of edge are numbered greater than ",
                   ...
                   "\n Check these: ", 
                   paste(
                       unique(tree$edge[tree$edge>expNodeNumber]),
                       collapse = " "
                       )
                   )
            )
    ...
    if(tree$Nnode>1){
        ...
        if(any(tabEdge[length(tree$tip.label) + 2:tree$Nnode]<2)){
            stop(paste0(
                "Some internal node IDs appear less than twice in $edge: ",
    ...
    if(any(tree$edge[,1]<(length(tree$tip.label) + 1))){
        stop(paste0(
            "Apparent tip IDs appear in column 1 of $edge: \n",
            tree$edge[tree$edge[,1]<(length(tree$tip.label) + 1),1],
            collapse = " "
            ))
        }
    #test edge matrix
