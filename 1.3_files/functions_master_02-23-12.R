...
timePaleoPhy<-function(tree,timeData,type="basic",vartime=NULL,ntrees=1,randres=FALSE,add.term=FALSE,rand.obs=FALSE,node.mins=NULL,plot=FALSE){
...
bin_timePaleoPhy<-function(tree,timeList,type="basic",vartime=NULL,ntrees=1,randres=FALSE,sites=NULL,add.term=FALSE,rand.obs=FALSE,node.mins=NULL,plot=FALSE){
...
srcTimePaleoPhy<-function(tree,timeData,sampRate,ntrees=1,anc.wt=1,node.mins=NULL,rand.obs=FALSE,FAD.only=FALSE,root.max=200,plot=FALSE){
	...
	for(ntr in 1:ntrees){
		...
		if(plot){
			par(mar=c(2.5,2.5,1,2.5));layout(matrix(1:3,3,));plot(ladderize(tree),show.tip.label=TRUE,use.edge.length=FALSE)
...
bin_srcTimePaleoPhy<-function(tree,timeList,sampRate,ntrees=1,sites=NULL,anc.wt=1,node.mins=NULL,rand.obs=FALSE,FAD.only=FALSE,root.max=200,plot=FALSE){
...
expandTaxonTree<-function(taxonTree,taxaData,collapse=NULL,plot=FALSE){
	...
	if(plot==TRUE){layout(matrix(1:2,,2));plot(taxonTree);plot(tree1)}
	return(tree1)
	}
...
getSampRateCont<-function(timeData,n_tbins=1,grp1=NA,grp2=NA,threshold=0.1,est_only=FALSE){
...
getSampProbDisc<-function(timeData,n_tbins=1,grp1=NA,grp2=NA,est_only=FALSE){
...
sampleRanges<-function(taxad,r,alpha=1,beta=1,rTimeRatio=0,min.taxa=2,ranges.only=TRUE,
	modern.samp.prob=1.0,minInt=0.01,plot.sampModel=F){
	...
	#rTimeRatio is the ratio of the latest taxon-avg sampling rates by the latest taxonavg sampling rates
	...
	#r<-c(0.2,0.1,0.3,0.4);alpha<-4;beta<-4;rTimeRatio<-0.01
	...
	if(length(rTimeRatio)!=1){if(length(rTimeRatio)!=length(names)){
		stop("Error: Multiple rTimeChange values input but not same length as number of taxa?")}}
	midpoint<-(max(timeData)+min(timeData))/2
	taxa.midpoints<-((timeData[,1]+timeData[,2])/2)

	if(length(rTimeChange)==1){rTimeChange<-rep(rTimeChange,length(names))}
	if(length(r)==1){		#get per-taxon r.avg for each taxon, if not input already
		taxa.timeChange<-midpoint-taxa.midpoints
	rTimeChange<-rTimeRatio/(max(timeData)-min(timeData))
	...
	if(any(r<=0)){stop("Error: rTimeChange causes some species-specific avg r <= 0")}
	if(any(alpha!=1) | any(beta!=1) | any(rTimeChange!=0)){		#get per species time vectors of r
		...
		rHatTime1<-lapply(rHatTime,function(x) ifelse(x<0,10^-3,x))
		rHatTime<-lapply(1:length(rHatTime),function(x) mean(rHatTime[[x]])*rHatTime1[[x]]/mean(rHatTime1[[x]]))
		}
	if(plot.sampModel){if(!is.null(rHatTime)){
		plot(0:1,c(0,max(unlist(rHatTime))),type="n",xlim=c(max(timeData),min(timeData)),
			xlab="Time (Time-Units before Present)",ylab="Instantaneous Sampling Rate (per Lmy)")
		for(i in 1:length(r)){lines(rangesTimes[[i]],rHatTime[[i]],lwd=2)}
	}else{
		plot(c(max(timeData),min(timeData)),c(r[1],r[1]),type="l",xlim=c(max(timeData),min(timeData)))
		}}
...
taxa2cladogram<-function(taxad,plot=FALSE){
...
timeSliceTree<-function(ttree,sliceTime,drop.extinct=FALSE,plot=TRUE){
	...
	if(plot){layout(matrix(1:2,,2));plot(ladderize(ttree),show.tip.label=FALSE);axisPhylo();plot(ladderize(stree1),show.tip.label=FALSE)}
	return(stree1)
	}
...
simFossilTaxa<-function(p,q,w=0,u=0,nruns=1,mintaxa=1,maxtaxa=1000,mintime=1,maxtime=1000,minExtant=0,maxExtant=NULL,min.cond=TRUE,print.runs=FALSE,plot=FALSE){
	...
	#min.cond 
	#p=0.1;q=0.1;w=0.1;u=0.1;nruns=10;mintaxa=1;maxtaxa=200;mintime=1;maxtime=100;minExtant=0;maxExtant=0;plot=TRUE;print.runs=TRUE;min.cond=TRUE
	#p=0.1;q=0.9;w=0;u=0;nruns=1;mintaxa=1;maxtaxa=100;mintime=1;maxtime=100;minExtant=0;maxExtant=0;plot=TRUE;print.runs=TRUE;min.cond=TRUE
	#set.seed(444);p=0.1;q=0.1;w=0;u=0;nruns=10;mintaxa=1;maxtaxa=1000;mintime=1;maxtime=100;minExtant=0;maxExtant=NULL;plot=TRUE;print.runs=TRUE;min.cond=FALSE
	#set.seed(444);p=0.1;q=0;w=0;u=0;nruns=1;mintaxa=1;maxtaxa=10;mintime=1;maxtime=10;minExtant=0;maxExtant=NULL;plot=TRUE;print.runs=TRUE;min.cond=TRUE
	#idiot proofing
	...
	for(i in 1:nruns){
		...
		while(any(is.na(taxad[,4])) & continue){
			...
			while(is.na(taxad[tpick,4]) & continue){
				...
				if(max(taxad[,3:4],na.rm=TRUE)>=mintime & ifelse(numext>0,nrow(taxad)>mintaxa,nrow(taxad)>=mintaxa)
		...
		colnames(taxad2)<-c("taxon.id","ancestor.id","orig.time","ext.time","still.alive")
		results[[i]]<-taxad2
		if(plot){
...
taxicDivCont<-function(timeData,int.length=1,int.times=NULL,plot=TRUE,plotLogRich=FALSE,timelims=NULL){
	...
	tblen<-int.length
	if(ncol(timeData)==5){timeData<-timeData[,3:4,drop=FALSE]}	#also allow it to accept taxad objects
	timeData<-timeData[!is.na(timeData[,1]),,drop=FALSE]
	if(any(is.na(timeData))){stop("Weird NAs in Data??")}
	if(any(timeData[,1]<timeData[,2])){stop("Error: timeData is not in time relative to modern (decreasing to present)")}
	if(any(timeData[,2]<0)){stop("Error: Some dates in timeData <0 ?")}
...
multiDiv<-function(data,int.length=1,plot=TRUE,split.int=TRUE,drop.ZLB=TRUE,plotLogRich=FALSE,timelims=NULL){
	...
	if(any(dclass1==1)){
		lims1<-sapply(data[dclass1==1],function(x) 
			if(ncol(x)==5){
				c(min(x[,3:4],na.rm=TRUE),max(x[,3:4],na.rm=TRUE))
			}else{c(min(x,na.rm=TRUE),max(x,na.rm=TRUE))}
			)
	}else{lims1<-NA}
	if(any(dclass1==2)){
	...
	if(any(dclass1==1)){
		divs1<-sapply(data[dclass1==1],function(x) 
			taxicDivCont(timeData=x,int.times=int.times,plot=FALSE)[,3])
		div<-cbind(div,divs1)
		}
	if(any(dclass1==2)){
...
simPaleoTrees<-function(p,q,r,ntrees=1,all.extinct=FALSE,modern.samp=FALSE,mintime=1,maxtime=100,mintaxa=2,maxtaxa=500,drop.zlb=TRUE,plot=FALSE){
	...
	for(i in 1:ntrees){
		...
		while(rerun){
			taxa<-suppressMessages(simFossilTaxa(p,q,w=0,u=0,nruns=1,mintaxa=mintaxa,maxtaxa=maxtaxa,
				maxtime=maxtime,maxExtant=ifelse(all.extinct,0,maxtaxa),min.cond=FALSE,plot=plot))
...
simFossilTaxa_SRCond<-function(r,avgtaxa,p,q,w=0,u=0,nruns=1,maxtime=1000,maxExtant=NULL,plot=FALSE){
