...
#' 
#' @details This function will partition the internode 
...
#' @param tree A dated phylogeny to be analyzed, as an object of class \code{phylo}, 
...
#' @param tol Tolerance used to distinguish extant taxa, 
...
#' @return The output is a list consisting of four vectors, with the labels of the vectors being their
...
branchClasses <- function(tree,whichExtant = NULL,tol = 0.01){
	...
	if(is.null(whichExtant)){
		...
		if(is.null(tree$root.time)){
		  ...
		  modern <- tree$root.time
		  }
		modern.tips <- which(dists1>(modern-tol))
		dead.tips <- which(dists1 <= (modern-tol))
	}else{
		if(length(whichExtant) != Ntip(tree)){
		  stop("Length of whichExtant is not equal to number of tip taxa on tree")
		  }
		modern.tips <- which(
		  sapply(tree$tip.label, function(x) 
		    any(x == names(which(whichExtant == 1)))
		    )
		  )
		dead.tips <- which(
		  sapply(tree$tip.label, function(x) 
		    any(x == names(which(whichExtant == 0)))
		    )
		  )
		}
	int.edges <- tree$edge[,2]>Ntip(tree)
	live.edges <- sapply(tree$edge[,2], function(x) 
	  any(x == modern.tips)
	  )
	dead.edges <- sapply(tree$edge[,2], function(x) 
	  any(x == dead.tips)
	  )
	if(is.null(tree$root.time)){
	...
	list(
	  brlen.all = brlen.all,
	  brlen.int = brlen.int,
	  brlen.live = brlen.live,
	  brlen.dead = brlen.dead
	  )
	}
