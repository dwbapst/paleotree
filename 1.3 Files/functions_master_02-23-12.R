...
getSampRateCont<-function(timeData,n_tbins=1,grp1=NA,grp2=NA,threshold=0.1,est_only=FALSE){
	...
	if(n_tbins>1){
		...
		if(g1s>0){
			...
			if(g2s>0){
				...
				res<-list(Title=title,SMax=SMax,AICc=aicc,mqrt=mqrt[,1:2],mqrg1=mqrg1[,1:2],
					mqrg2=mqrg2[,1:2],t_ends=t_ends,binlen=t_bl,convergence=conv,message=mes)
			}else{
				res<-list(Title=title,SMax=SMax,AICc=aicc,mqrt=mqrt[,1:2],
					mqrg1=mqrg1[,1:2],t_ends=t_ends,binlen=t_bl,convergence=conv,message=mes)
				}
		}else{
			res<-list(Title=title,SMax=SMax,AICc=aicc,mqrt=mqrt,
				t_ends=t_ends,binlen=t_bl,convergence=conv,message=mes)
			}	
	}else{
		mqr<-matrix(par,,2,byrow=TRUE)
		Pp<-mqr[,2]/(mqr[,1]+mqr[,2])
		...
		if(g1s>0){
			...
			if(g2s>0){
				mqrg2<-mqr[(g1s+1):(g1s+g2s),]
				res<-list(Title=title,SMax=SMax,AICc=aicc,mqrg1=mqrg1[,1:2],mqrg2=mqrg2[,1:2],convergence=conv,message=mes)
			}else{
			...
			res<-list(Title=title,pars=c(qMax=qMax,rMax=rMax,Comp=Pp),SMax=-SMax,AICc=aicc,convergence=conv,message=mes)
			}
		}}
	return(res)
	}
		...
		if(n_tbins>1){
			...
			if(g1s>0){
				...
				if(g2s>0){
					...
					res<-list(Title=title,SMax=SMax,AICc=aicc,mqRg1=mqRg1,convergence=conv,message=mes)
					}
			}else{
				RMax<-mqR[,2]
				qMax<-mqR[,1]
...
sampleRanges<-function(taxad,r,min.taxa=2,ranges.only=TRUE,modern.samp=TRUE){
	...
	#if modern.samp=TRUE, then all still-living taxa (taxa at 0 for LAD) are ALWAYS last observed at zero
...
taxicDivCont<-function(timeData,int.length=1,int.times=NULL,plot=TRUE,plotLogRich=FALSE){
...
taxicDivDisc<-function(timeList,int.times=NULL,plot=TRUE,plotLogRich=FALSE){
...
phyloDiv<-function(tree,int.length=1,int.times=NULL,plot=TRUE,plotLogRich=FALSE,drop.ZLB=TRUE){
...
multiDiv<-function(data,int.length=1,plot=TRUE,drop.ZLB=TRUE,plotLogRich=FALSE){
	...
	if(any(dclass1==2)){
		divs2<-sapply(data[dclass1==2],function(x) 
			taxicDivDisc(timeList=x,int.times=int.times,plot=FALSE)[,3])
		div<-cbind(div,divs2)
		}
	if(any(dclass1==3)){
...
plotMultiDiv<-function(results,plotLogRich=FALSE){
